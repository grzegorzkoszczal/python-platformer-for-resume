...
class Menu:
    ...
    def set_difficulty(self, value, difficulty):
        pass
        # TO DO: Implement the Normal and Hard difficulties with differences of time
        # given in order to complete level and health points given for entire level.

    def menu_screen():
        pygame_menu.Menu("Welcome", 400, 300, theme=pygame_menu.themes.THEME_BLUE)
...
def main(window):
    ...
    while run:
        ...
        # Shifting the canvas left and right if the player model approaches the horizontal edges of platform
        if (
            player.rect.left - offset_x <= scroll_area_width and player.x_velocity < 0
        ) or (
            player.rect.right - offset_x >= WIDTH - scroll_area_width
            and player.x_velocity > 0
        ):
            offset_x += player.x_velocity

        # Shifting the canvas up and down if the player model approaches the vertical edges of platform
        if (
            player.rect.top - offset_y <= scroll_area_height and player.y_velocity < 0
        ) or (
            player.rect.bottom - offset_y >= HEIGHT - scroll_area_height
            and player.y_velocity > 0
        ):
            offset_y += player.y_velocity
